...
void StartScreen::load()
{
	...
	eManager->Subscribe(Events::EventType::Event_KeyPress, [&](Events::Event e)
	{
		...
		if (e.data == GLFW_KEY_ENTER && c)
			...
			else
			{
				...
				{
					tween.CreateTween(&bump->transform, Render::Rect(.5, 1.2, bump->transform), 0.65, outCubic, [&]() {
						tween.Clear();
	...
	bump = new Sprite(.5, 2, Average4K::skin->GetTexture("Menu/avg4k"));
	bump->center = true;
	addObject(bump);
	bump->setRatio(true);
...
void StartScreen::draw()
{
	...
	if (beat >= _eBeat && !started)
	{
		...
		tween.CreateTween(&bump->transform, Render::Rect(.5, .5, bump->transform), 1, outCubic, NULL);
	}
