...
// hehe
int pixelSize(SDL_Renderer* renderer, Sint16 x, Sint16 y, Sint16 size)
{
	SDL_Rect rect;
	rect.x = x;
	rect.y = y;
...
{
	Uint8 *c = (Uint8 *)&color; 
	return roundedRectangleRGBA(renderer, x1, y1, x2, y2, rad, c[0], c[1], c[2], c[3], 1);
}
...
\returns Returns 0 on success, -1 on failure.
*/
int roundedRectangleRGBA(SDL_Renderer * renderer, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, Sint16 rad, Uint8 r, Uint8 g, Uint8 b, Uint8 a, Sint16 borderSize)
{
	int result = 0;
	Sint16 tmp;
	Sint16 w, h;
...
{
	Uint8 *c = (Uint8 *)&color; 
	return arcRGBA(renderer, x, y, rad, start, end, c[0], c[1], c[2], c[3], 1);
}
	...
	*/
	do {
		...
		if (cx > 0) {
			...
			if (drawoct & 4)  result |= pixelSize(renderer, xmcx, ypcy, size);
			if (drawoct & 2)  result |= pixelSize(renderer, xpcx, ypcy, size);
			if (drawoct & 32) result |= pixelSize(renderer, xmcx, ymcy, size);
			if (drawoct & 64) result |= pixelSize(renderer, xpcx, ymcy, size);
		} else {
			if (drawoct & 96) result |= pixelSize(renderer, x, ymcy, size);
			if (drawoct & 6)  result |= pixelSize(renderer, x, ypcy, size);
		}
		...
		if (cx > 0 && cx != cy) {
			ypcx = y + cx;
			ymcx = y - cx;
			if (drawoct & 8)   result |= pixelSize(renderer, xmcy, ypcx, size);
			if (drawoct & 1)   result |= pixelSize(renderer, xpcy, ypcx, size);
			if (drawoct & 16)  result |= pixelSize(renderer, xmcy, ymcx, size);
			if (drawoct & 128) result |= pixelSize(renderer, xpcy, ymcx, size);
		} else if (cx == 0) {
			if (drawoct & 24)  result |= pixelSize(renderer, xmcy, y, size);
			if (drawoct & 129) result |= pixelSize(renderer, xpcy, y, size);
		}
