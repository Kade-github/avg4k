...
namespace Average4k
{
	...
	public:
		...
		File GetDefaults()
		{
			...
			nf.settings.push_back({ "Right Keybind", "K",  "K", S_String, -1,-1 });
			...
			nf.settings.push_back({ "Resolution", "max",  "max", S_String, -1,-1 });
			nf.settings.push_back({ "Display", "Borderless",  "Borderless", S_String, 0,-1  });
